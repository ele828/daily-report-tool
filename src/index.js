...
function sendEmail(content) {
  ...
  const account = config && config.email
    ? config.email.account : '';

  const password = config && config.email
...
const types = [BREAKING_CHANGE, NEW_FEATURE, UI_CHANGE, BUGFIX, REFACTOR, TOOLING, OTHER];
...
async function getCommits() {
  const resp = await axios.get('https://api.github.com/repos/ringcentral/ringcentral-js-widgets/commits?since=2018-6-9T00:00:00Z&until=2018-06-14T:00:00Z');
  const prs = [];
  for (const data of resp.data) {
    const sha1 = data.sha;
    console.log('sha1:', sha1);
    await sleep(2000);
    const prResp = await axios.get(`https://api.github.com/search/issues?q=${sha1}`);
